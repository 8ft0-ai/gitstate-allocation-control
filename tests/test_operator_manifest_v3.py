...
from phase2.operator_guard import (
    GuardObservation,
    GuardObservationV3,
    evaluate_guards,
)
...
def make_manifest():
    ...
    proposal_records = parse_governance_comments_v2(
        [proposal], expected_owner="8ft0-ai", expected_issue=ISSUE
    )
    baseline = governance_history_baseline(proposal_records)
...
def witness_comment(manifest, proposal, preflight, approval, *, run_attempt=1):
    payload = {
        ...
        "proposal": binding(proposal),
        "manifest": {
            "comment_id": MANIFEST_COMMENT_ID,
...
def authority_comment(manifest, proposal, approval, witness, readiness):
    return governance_comment(
        800,
        governance_payload(
            "authority",
...
def complete_chain():
    ...
    authority = authority_comment(
        manifest, proposal, approval, witness_raw, readiness
    )
...
class OperatorManifestV3Tests(unittest.TestCase):
    def test_v3_is_additive_pre_readiness_and_pre_authority(self):
        ...
        with self.assertRaisesRegex(
            OperatorContractError, "V3_MANIFEST_HAS_NO_READINESS"
        ):
            _ = parsed.readiness
        with self.assertRaisesRegex(
    ...
    def test_transition_witness_is_exact_attempt_one_capability_denied_pass(self):
        ...
        self.assertEqual(parsed.preflight_evidence, type(parsed.preflight_evidence)(
            PREFLIGHT_COMMENT_ID, sha256_text(preflight["body"])
        ))

        with self.assertRaisesRegex(TransitionWitnessError, "ATTEMPT_INVALID"):
            parse_transition_witness(
                witness_comment(
                    manifest, proposal, preflight, approval, run_attempt=2
                ),
                expected_owner="8ft0-ai",
                expected_issue=ISSUE,
                expected_operation=OPERATION,
    ...
    def test_live_requires_exact_witness_readiness_and_manifest_bound_authority(self):
        ...
        bad_history = history(
            manifest, [proposal, approval, bad_readiness, bad_authority]
        )
