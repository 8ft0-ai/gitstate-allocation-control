...
the Beads `issues` and `labels` tables rather than the isolated SQLite fixture.
Eligibility consumes the pinned Beads-maintained `issues.is_blocked` readiness
materialisation, so conditional blockers, waits/gates, wisp targets and propagated
blocking semantics remain Beads authority rather than being partially
reimplemented by the allocator. `capability:*` labels remain the request
capability filter. A grant inserts its request, allocation and active uniqueness
...
expected-old Git CAS identity: the adapter probes the backing ordinary Git URL
with `git ls-remote`, normalises the repository URL using the pinned Beads v1.1.0
Git-to-Dolt rules, and performs a fresh `dolt clone` through the corresponding
`git+` transport into an isolated temporary database. Before opening the
caller-supplied PEP-249 connection, it reads the cloned Dolt `HEAD` through the
CLI. Bootstrap then fails closed unless the opened connection reports that exact
`DOLT_HASHOF('HEAD')` and the expected `main` branch. Ref movement while the
clone is in flight invalidates the snapshot.
...
Beads non-force publication path: no force argument exists, and canonical data is
never published by checking out or committing `refs/dolt/data` as an ordinary
Git worktree. A failed push is classified as stale if the remote CAS identity
moved and otherwise as a canonical push failure; a successful push must advance
the canonical Git ref before an accepted identity is returned. Bounded stale
retries always start from a fresh Dolt clone.
...
Credential-free repository-native integration validation pins Beads v1.1.0 and
the exact standalone Dolt v2.1.4 dependency by release SHA-256, applies this DDL
to a temporary Git-backed Beads database, and exercises real foreign keys,
append-only triggers, grant/release ownership mirroring, `issues.is_blocked`
readiness and stale expected-old-SHA rejection. No live canonical repository or
allocator credential is required or selected by those tests.
