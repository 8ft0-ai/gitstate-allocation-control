...
class GuardObservationV3(_v1.GuardObservation):
...
def _evaluate_v3_governance(
    ...
    try:
        if observation.stage == "preflight":
            ...
            if not isinstance(observation, GuardObservationV3):
                return GuardResult.failure("GOVERNANCE_RECORD_INVALID")
            reduce_governance_history_v3(
                manifest,
                observation.governance_history,
                manifest_comment_id=manifest.manifest_comment_id,
                transition_witness=observation.transition_witness,
                stage="live",
            )
    except GovernanceStateError as exc:
        return GuardResult.failure(exc.code)
...
def _evaluate_v3_boundaries(
    ...
    governance = _evaluate_v3_governance(manifest, observation)
...
def evaluate_guards(manifest: ExecutionManifest, observation: GuardObservation) -> GuardResult:
    if isinstance(manifest, GuardedExecutionManifestV3):
        return _evaluate_v3_boundaries(manifest, observation)
