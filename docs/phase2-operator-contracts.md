...
The live path is split into two semantic gates using the same B1 pure evaluator. Gate L1 runs in a capability-denied job before the protected environment and has no allocator private key, App JWT, installation token, control/state mutation token or mutation provider. Gate L2 can run only after L1 PASS. The protected live wrapper reads the allocator App key once after the fresh L1 semantic pass, removes it from the real process environment and all guard/legacy mappings, and passes it only as an explicit one-use input for App JWT creation. Git, Beads and Dolt child-process environments defensively exclude that key. L2 then mints one installation-wide Metadata-read inventory token, completely proves the exact selected repository set, and positively revokes that token before the existing reduced control/state token profiles may be minted. The public carrier/protected-main fence is repeated through L1 and L2 so a newly effective invalidation cannot be outrun by stale eligibility.
