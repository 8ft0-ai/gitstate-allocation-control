...
def make_state(*, readiness_disposition="ready", owner_observation=None):
    ...
    return proposal, readiness, authority, manifest, comments, observation
...
class OperatorGuardTests(unittest.TestCase):
    ...
    def test_stale_readiness_binding_is_caught_without_prose_matching(self):
        ...
        records = parse_governance_comments(
            [proposal, stale_readiness, authority],
            expected_owner="8ft0-ai",
            expected_issue=40,
        )
        result = evaluate_guards(
            manifest,
            with_observation(observation, governance_records=records),
        )
        self.assertFalse(result.passed)
        self.assertEqual(result.code, "GOVERNANCE_RECORD_INVALID")
        self.assertEqual(result.category, "authority_security")

    def test_explicit_revocation_and_consumption_fail_closed(self):
        _, _, _, manifest, comments, observation = make_state()
    ...
    def test_second_simultaneously_active_authority_is_ambiguous(self):
        proposal, readiness, authority, manifest, comments, observation = make_state()
    ...
    def test_live_stages_require_exact_manifest_approval(self):
        ...
        live = with_observation(
            ...
            stage="live_l1",
            governance_records=records,
            manifest_approval=type(manifest.proposal)(approval["id"], sha256_text(approval["body"])),
        )
        self.assertTrue(evaluate_guards(manifest, live).passed)

        missing = with_observation(live, manifest_approval=None)
        self.assertEqual(evaluate_guards(manifest, missing).code, "AUTHORITY_NOT_GRANTED")

        wrong = with_observation(
            live,
            manifest_approval=type(manifest.proposal)(approval["id"], "0" * 64),
        )
    ...
    def test_mutable_identity_failures_are_typed_and_do_not_refresh_authority(self):
        ...
        cases = [
            ...
            (
                {
                    "operator_history": type(manifest.operator_history)(
                        manifest.operator_history.through_id,
                        "9" * 64,
                    )
                },
            ...
            (
                {
                    "workflow_history": type(manifest.workflow_history)(
                        manifest.workflow_history.through_id,
                        "9" * 64,
                    )
                },
    ...
    def test_read_evidence_statuses_and_owner_observation_fail_closed(self):
        ...
        result = evaluate_guards(manifest, observation)
        self.assertEqual(result.code, "READ_EVIDENCE_UNAVAILABLE")
