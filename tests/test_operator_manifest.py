...
def lineage_comments():
    proposal = governance_comment(
        ...
        human="proposal without any historical capsule literal",
    )
    ...
    authority = governance_comment(
        ...
        human="authority semantics do not serialize old capsule or run identities",
    )
...
def manifest_payload(proposal, readiness, authority, **changes):
    value = {
        ...
        "executor": {
            ...
            "module_blobs": [
                {"path": "phase2/operator_runtime.py", "blob_sha": MODULE_BLOB},
            ],
        },
...
def make_manifest(**changes):
    ...
    value = manifest_payload(proposal, readiness, authority, **changes)
    raw = canonical_json(value)
    return proposal, readiness, authority, raw, parse_execution_manifest(raw)


def v1_capsule_comment(comment_id=201, *, capsule_id="b" * 32, workstream_e=False):
    payload = {
...
def v1_consumption_comment(capsule, comment_id=202, *, trusted_sha=CONTROL_SHA, run_attempt=1):
    capsule_payload = __import__("json").loads(capsule["body"][len(V1_CAPSULE_PREFIX) :])
    payload = {
...
class OperatorManifestTests(unittest.TestCase):
    def test_manifest_is_canonical_exact_and_digest_bound(self):
        ...
        self.assertEqual(parsed.payload["contract"], MANIFEST_CONTRACT)
        ...
        with self.assertRaisesRegex(OperatorContractError, "NONCANONICAL_JSON"):
            parse_execution_manifest(noncanonical)
    ...
    def test_v1_history_remains_read_only_compatible_and_canonical(self):
        capsule = v1_capsule_comment()
        consumption = v1_consumption_comment(capsule)
        records = parse_v1_operator_history([capsule, consumption])
        self.assertEqual(len(records), 2)
        canonical = canonical_operator_history(records)
        expected = (
    ...
    def test_v1_history_fails_closed_on_replay_orphan_mismatch_and_workstream_e(self):
        capsule = v1_capsule_comment()
        consumption = v1_consumption_comment(capsule)
        ...
        duplicate_capsule = v1_capsule_comment(203)
        with self.assertRaisesRegex(OperatorContractError, "OPERATOR_HISTORY_DUPLICATE_CAPSULE"):
            parse_v1_operator_history([capsule, duplicate_capsule])

        second_consumption = v1_consumption_comment(capsule, 204)
        with self.assertRaisesRegex(OperatorContractError, "OPERATOR_HISTORY_DUPLICATE_CONSUMPTION"):
            parse_v1_operator_history([capsule, consumption, second_consumption])

        mismatch = v1_consumption_comment(capsule, trusted_sha="9" * 40)
        with self.assertRaisesRegex(OperatorContractError, "OPERATOR_HISTORY_CONSUMPTION_MISMATCH"):
            parse_v1_operator_history([capsule, mismatch])

        rerun = v1_consumption_comment(capsule, run_attempt=2)
        with self.assertRaisesRegex(OperatorContractError, "OPERATOR_HISTORY_CONSUMPTION_MISMATCH"):
            parse_v1_operator_history([capsule, rerun])
    ...
    def test_workflow_history_binds_dispatch_identity_not_stale_terminal_conclusions(self):
        ...
        changed_run = [WorkflowHistoryRecord(102, 1, CONTROL_SHA, "live_scenario_suite", "failure")]
        self.assertNotEqual(
            workflow_history_baseline(failure).history_sha256,
            workflow_history_baseline(changed_run).history_sha256,
        )
